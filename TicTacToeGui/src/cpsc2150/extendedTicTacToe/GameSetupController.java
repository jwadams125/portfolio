...
public class GameSetupController {
    ...
    public void processButtonClick(int rows, int cols, int players, int numWin) {
        String errorMsg = "";
        if (rows < IGameBoard.MINROWSCOLS || rows > max_size) {
            errorMsg += "Rows must be between " + IGameBoard.MINROWSCOLS + " and " + max_size;
        }

        if (cols < IGameBoard.MINROWSCOLS || cols > max_size) {
            errorMsg += "Columns must be between " + IGameBoard.MINROWSCOLS + " and " + max_size;
        }
        ...
        if (numWin < IGameBoard.MINTOWIN) {
            errorMsg += "Number to win must be at least " + IGameBoard.MINTOWIN;
        }
